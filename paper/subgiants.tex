...
with individual oscillation mode measurements published in
\citet{Appourchaux2012}.
These stars have a range of masses and radii.
...
temperature: the stellar envelope cools and gets slightly less luminous as the
surface layer moves away from the centre of the star.
It gets brighter again as the surface moves closer to the stellar centre.
These surface variations do not only produce luminosity fluctuations, they
also cause variations in radial velocity.
We used equation 5 in \citet{Kjeldsen1995} which relates the fractional
variation in bolometric luminosity to radial velocity in order to transform
the {\it Kepler} light curves into RV curves:
\begin{equation}
...
proceeded to fit a linear combination of sine waves to the time series, using
the oscillation frequencies published in \citet{Appourchaux2012}.
The resulting amplitudes were then used to generate synthetic data on which
we could train our observing strategy.
...
% section heading temporarily stolen from Dumusque
